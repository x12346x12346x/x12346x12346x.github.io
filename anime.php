...
if(isset($_POST['title']) & isset($_POST['content']) && $_POST['title'] && $_POST['content']){
    ...
    $title = $_POST['title'];
    $sql = "SELECT * FROM `article` WHERE category='anime' AND title='$title'";
    $result = mysql_query($sql);
    if(mysql_num_rows($result) == 0){
    	echo '<script> alert("沒有這篇文章！") </script>';
    }
    else{
    	$sql = "INSERT INTO `response`(`category`, `title`, `name`, `content`) VALUES ('anime','$title','$name','$content')";
    	mysql_query($sql);
    }
    echo '<meta http-equiv=REFRESH CONTENT=0;url=anime.php>';
}
?>
...
<?php
$sql = "SELECT * from article WHERE category='anime'";
$article = mysql_query($sql);
while($row_a = mysql_fetch_array($article)){
	echo "<hr>";
    showArticle($row_a);
    $title = $row_a['title'];
    $sql = "SELECT * from response WHERE category='anime' AND title='$title'";
    $res = mysql_query($sql);
